...
if ( PROTOCOL == "FLASH" ):
	...
	# Peak Annotation
	include: config["rules"] + "/PARCLIP/PARCLIP_misc.py"
	# Motif Detection
	include: config["rules"] + "/Main/motif_detection.py"
	...
	if ( demultiplexed == "yes" ):
		rule all:
			input: 
				...
				expand(COVERAGE_OUTDIR + "/bigwig/{sample}_{replicate}_coverage_{type}_strand.bigwig", sample=SAMPLES[1], replicate=REP_NAME_CONTROL, type=["pos", "neg", "both"]),
